...
let rec
const midiinputs = []
let popmeloval = 1 // twinkle as standard
const temprange = { min: 0.4, max: 1.6 } // set range for 5 options
let bpm = 120
...
let colorscale = 0 // start with temperature
...
let views = [false, true, false, false, false]
  ...
  async function playRNN () {
    try {
      if (player !== undefined) {
        ...
        // var datalength = dataLength(tt);
        await getBrush()
        await currentMeloView.notes.forEach((element, i) => {
  ...
  */
  ...
  async function initRecorder(e){
    ...
    rec.callbackObject = {
      ...
      noteOn: (p,v,d) => {
        synth.triggerAttack(MIDI_NOTES.find(obj => {return obj.pitch === p}).label)
      },
      noteOff: (p,v,d) => {
        synth.triggerRelease(MIDI_NOTES.find(obj => {return obj.pitch === p}).label)
      },
      stop: () => {
        numnotesrecorded = 0
      }
      ...
      */}{/**
          ...
          <select className="button" id="instrument" onChange={(value) => {setInstrument()}}>
            ...
            <option key={6} value={6}>Double Bass</option>
              ...
              <div id='ird' style={{ display: 'none' }}>
                <Grid item minWidth={(103 / partialGrid) + '%'} maxHeight='77%'>
                  {/** <div>
        <div style={{textAlign: "right"}}>
      ...
      <FontAwesomeIcon className="icon" icon={faInfo} onMouseEnter={(e)=>{tooltip("infoIcicle","infoIcicle")}} onMouseLeave={(e)=>{tooltip("infoIcicle","infoIcicle")}}></FontAwesomeIcon>
        ...
        */}
              ...
              <div id='nld' style={{ display: 'none' }}>
                <Grid item minWidth={(103) + '%'} maxHeight='75%'>
                  {/** <div>
        <div style={{textAlign: "right"}}>
        <div className="infotip">
          <span className="infotiptext" id="infoIcicle" onMouseEnter={tooltip("infoIcicle","click")}>
      ...
      <FontAwesomeIcon className="icon" icon={faInfo} onMouseEnter={(e)=>{tooltip("infoIcicle","infoIcicle")}} onMouseLeave={(e)=>{tooltip("infoIcicle","infoIcicle")}}></FontAwesomeIcon>
        <button className="button" id="bird" onClick={(e)=>setMinMax("ird")} >- Icicle</button>
                  ...
                  <NodeLinkRolls closeView={closeView} partialGrid={partialGrid} display={views[1]} toggleState={2} width={(window.innerWidth / partialGrid) - gridMargin} height={window.innerHeight * 0.65} modelUsedFlag={modelUsedFlag} data={mainmelo} octaveRange={octaveRange} darkmode={darkmode} callback={setMeloIcicle} getBPM={() => { return bpm }} />
        ...
        */}
              <div id='simd' style={{ display: 'none' }}>
                <Grid item minWidth={(103 / partialGrid) + '%'} maxHeight='70%'>
                  {/* <div id="ccd" >
          ...
          <div className="content-tabs">
            ...
            > */}
              <div id='cord' style={{ display: 'none' }}>
                <Grid item minWidth={(103) + '%'} maxHeight='60%'>
          ...
          <Box sx={{ filter: 'drop-shadow( 0px 0px 7px #aaa)', position: 'fixed', backgroundColor: 'toolbar.main', height: '4%', bottom: 0, left: 0, right: 0 }}>
        {/**     <FormControlLabel sx={{ ml: '50px', mr: '50px' }} control={<Switch checked={provVis} color='secondary' onClick={() => { toggleView(5) }} />} label='Provenance' labelPlacement='end' />
            <FormControlLabel sx={{ ml: '50px', mr: '50px' }} control={<Switch checked={views[0]} color='secondary' onClick={() => { toggleView(0) }} />} label='Icicle' labelPlacement='end' />
        */}    <FormControlLabel sx={{ ml: '50px', mr: '50px' }} control={<Switch checked={views[1]} color='secondary' onClick={() => { toggleView(1) }} />} label='Node-Link' labelPlacement='end' />
            <FormControlLabel sx={{ ml: '50px', mr: '50px' }} control={<Switch checked={views[2]} color='secondary' onClick={() => { toggleView(2) }} />} label='Similarity' labelPlacement='end' />
        {/*    <FormControlLabel sx={{ ml: '50px', mr: '50px' }} control={<Switch checked={views[3]} color='secondary' onClick={() => { toggleView(3) }} />} label='Correlation' labelPlacement='end' />
